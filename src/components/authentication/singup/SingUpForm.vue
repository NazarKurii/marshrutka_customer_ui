  ...
  >
    ...
    <div class="flex gap-[20px]">
      <Input
        ...
        @model="
          (firstName: string) => {
            singup.user.firstName = firstName
    ...
    >
      <p class="text-red-500 text-lg text-center">
        {{ t('authentication.singup.problem') }}
      </p>
...
import { identifyInvalidParams, validate } from '@/scripts/authentication'
...
async function sinup() {
  ...
  if (validation.value.isValid) {
    ...
    const problem = await singup.do()
    invalidMessages.value = []
    if (problem) {
      setTimeout(() => {
        requestProblem.value = problem
        requesting.value = false
        const params = identifyInvalidParams(problem)
